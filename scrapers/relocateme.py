"""
Relocate.me scraper — relocate.me
Specifically lists jobs that include relocation packages.
Good fit since Ophek is relocating from Tel Aviv to Berlin.
"""

from __future__ import annotations

import time
from typing import Iterator
from urllib.parse import quote

from scrapers.base import BaseScraper, JobListing


class RelocateMeScraper(BaseScraper):
    source_key = "relocateme"

    def scrape(self, query: str) -> Iterator[JobListing]:
        url = f"https://relocate.me/search?query={quote(query)}&location=Germany"
        self.page.goto(url, timeout=30_000)
        time.sleep(3)

        for _ in range(3):
            self.page.keyboard.press("End")
            time.sleep(1.5)

        cards = self.page.query_selector_all(
            ".job-card, [class*='JobCard'], article.job"
        )

        for card in cards:
            try:
                title_el = card.query_selector("h2, h3, .job-title, [class*='title']")
                company_el = card.query_selector(".company, [class*='company'], [class*='Company']")
                location_el = card.query_selector(".location, [class*='location']")
                link_el = card.query_selector("a[href]")

                if not (title_el and link_el):
                    continue

                href = link_el.get_attribute("href") or ""
                if not href.startswith("http"):
                    href = "https://relocate.me" + href

                listing = JobListing(
                    source=self.source_key,
                    title=self._clean(title_el.inner_text()),
                    company=self._clean(company_el.inner_text()) if company_el else "Unknown",
                    location=self._clean(location_el.inner_text()) if location_el else "Germany",
                    url=href.split("?")[0],
                )

                if not self._passes_filters(listing):
                    continue

                description = self._fetch_description(href)
                listing.description = description

                if not self._passes_filters(listing):
                    continue

                yield listing

            except Exception:
                continue

    def _fetch_description(self, url: str) -> str:
        try:
            self.page.goto(url, timeout=20_000)
            time.sleep(2)
            desc_el = self.page.query_selector(
                ".job-description, [class*='description'], [class*='Description']"
            )
            return self._clean(desc_el.inner_text()) if desc_el else ""
        except Exception:
            return ""
